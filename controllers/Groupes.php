...
class Groupes extends Controller {
   ...
   public function traitement($id, $arg) {
      ...
      if ($submit == "enregistrer" || $submit == "enregistreretfermer") {
         if ($arg) {
            ...
            $data = array('nom' => $post['nom'],
                ...
                'id_reference' => $post['groupe_reference'],
      ...
      if ($submit == "ajouter") {
         $this->model->table = "groupes";
         if ($this->model->insertion(array('nom' => $post['nom'],
                     'description' => "",
                     "interface" => $post['interfaces'],
                     'id_reference' => $post['groupe_reference'],
                     'est_archive' => 0))) {
            ...
            if ($this->model->insertion(array('controller' => 'accueil', 'droit' => 7, 'id_groupe' => $lastID))) {
      ...
      if ($submit == "supprimer") {
         ...
         $this->model->table = "droits";
         if ($this->model->delete(array('id_groupe' => $arg))) {
            $this->model->table = "groupes";
   ...
   public function formGroupe($id) {
      ...
      $this->model->table = "types";
      $data['types'] = $this->model->lecture();
