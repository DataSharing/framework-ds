...
class Parametres extends Controller
{
    ...
    public function traitements()
    {
        ...
        if ($submit == 'modifierGeneral') {
            $data = array(
                'nom_du_site' => $post['nom_du_site'],
                'base_url' => $post['base_url'],
                'langage' => $post['langues'],
                'controller_principal' => $post['controllers']
            );
            if ($this->majFichier($data, 'config')) {
                $this->model->log($this->session->utilisateur, get_class($this), FORM_ENREGISTRER . " [config.php]", 0, $this->session->id);
                $this->redirect('parametres/succes');
            } else {
                $this->model->log($this->session->utilisateur, get_class($this), FORM_ENREGISTREMENT_DONNEES_ERREUR . ' [config.php]', 0, $this->session->id);
            ...
            if ($this->TestConnection($post['hostname'], $post['namebdd'], $post['userbdd'], $post['passbdd'])) {
                ...
                if ($this->majFichier($data, 'database')) {
                    $this->model->log($this->session->utilisateur, get_class($this), FORM_ENREGISTRER . "[database.php]", 0, $this->session->id);
                    $this->redirect('parametres/succes');
                } else {
                    $this->model->log($this->session->utilisateur, get_class($this), FORM_ENREGISTREMENT_DONNEES_ERREUR . " [database.php]", 0, $this->session->id);
                    $this->view('app/erreurs/notification', FORM_ENREGISTREMENT_DONNEES_ERREUR);
                }
            } else {
            ...
            $data = array(
                'url_cas' => isset($post['url_cas']) ? $post['url_cas'] : '',
                'get_cas' => isset($post['get_cas']) ? $post['get_cas'] : '',
                'mode' => isset($post['mode'][0]) ? $post['mode'][0] : '',
                'port_cas' => isset($post['port_cas']) ? $post['port_cas'] : ''
            );
            if ($this->majFichier($data, 'auth')) {
                $this->model->log($this->session->utilisateur, get_class($this), FORM_ENREGISTRER . " [auth.php]", 0, $this->session->id);
                $this->redirect('parametres/succes');
            } else {
                $this->model->log($this->session->utilisateur, get_class($this), FORM_ENREGISTREMENT_DONNEES_ERREUR . "[auth.php]", 0, $this->session->id);
                $this->view('app/erreurs/notification', FORM_ENREGISTREMENT_DONNEES_ERREUR);
            }
        }
    }
    ...
    public function FormLogs()
    {
        ...
        $data['logs'] = $this->model->lecture(['SQL_CALC_FOUND_ROWS *'], [], '', array('date_modification' => 'desc'), $limit);
        $data['nb'] = $this->model->foundRows();
        $data['pagination'] =  $this->form->pagination($data['nb'], $page, $par_page, [], '', strtolower(__class__));
