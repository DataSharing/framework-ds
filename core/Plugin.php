...
Class Plugin extends Controller{
	...
	public function _initPlugins(){
		...
		foreach ($plugins as $plugin) {
			if($plugin['is_activated'] && $plugin['is_installed']){
				include dirname(__FILE__).'/../plugins/'.$plugin['name'].'/setup.php';
				$app = New Setup();
				$params = $app->_init();
	...
	public function install($name){
		include dirname(__FILE__).'/../plugins/'.$name.'/setup.php';
		$plugin = New Setup();
		$tables = $plugin->install();
	...
	public function dataPlugin($name){
		include dirname(__FILE__).'/../plugins/'.$name.'/plugin.php';
